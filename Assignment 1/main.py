...
r_o = np.c_[-b.sum(),np.c_[fr(1)*np.zeros((1,2*n+ns),dtype=fr),fr(1)*np.ones((1,m),dtype=fr)] - np.matmul(fr(1)*np.ones((1,m),dtype=fr), A)]
M = np.c_[b,A]
...
for i in range(m):
    cB[0,i]=c[0,B[i]]
r_o = np.c_[fr(0),c]-np.matmul(cB,M)
